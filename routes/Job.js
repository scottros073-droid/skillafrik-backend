...
// Assign a worker to a job (company only)
// ------------------------
...
// Create escrow payment for hiring
// ------------------------
...
// ------------------------
router.post('/:jobId/release', auth, async (req, res) => {
  try {
    const { jobId } = req.params;
    const userId = req.user.id;
    ...
    if (!job.escrow || job.escrow.status !== 'HELD') {
      return res.status(400).json({ message: 'Escrow not held or already released' });
    }
